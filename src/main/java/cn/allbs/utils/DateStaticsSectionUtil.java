...
public class DateStaticsSectionUtil {
    ...
    public Map<String, BigDecimal> minuteSection(LocalDateTime startTime, LocalDateTime endTime, int interval) {
        ...
        while (startTime.isBefore(endTime)) {
            result.put(startTime.format(DateTimeFormatter.ofPattern(DatePattern.NORM_DATETIME_MINUTE_PATTERN)), new BigDecimal(0));
    ...
    public Map<String, BigDecimal> minuteSection(LocalDateTime startTime, LocalDateTime endTime) {
        return minuteSection(startTime, endTime, 1);
    }
    ...
    public Map<String, BigDecimal> daySection(LocalDate startTime, LocalDate endTime) {
        ...
        Map<String, BigDecimal> result = new LinkedHashMap<>(num);
        while (startTime.isBefore(endTime)) {
            result.put(startTime.format(DateTimeFormatter.ofPattern(DatePattern.NORM_DATE_PATTERN)), new BigDecimal(0));
    ...
    public Map<String, BigDecimal> dayHour(LocalDate date, int interval) {
        ...
        while (startTime.isBefore(endTime)) {
            result.put(startTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH")), new BigDecimal(0));
    ...
    public Map<String, BigDecimal> monthDay(int year, int month) {
        ...
        while (startDate.isBefore(endDate)) {
            result.put(startDate.format(DateTimeFormatter.ofPattern(DatePattern.NORM_DATE_PATTERN)), new BigDecimal(0));
    ...
    public Map<String, BigDecimal> yearMonth(int year) {
        ...
        for (int i = 1; i <= 12; i++) {
            ...
            result.put(date.format(DateTimeFormatter.ofPattern(DatePattern.NORM_MONTH_PATTERN)), new BigDecimal(0));
        }
        return result;
    }
}
